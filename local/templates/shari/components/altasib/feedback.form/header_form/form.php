...
<? $typeQuestionName = "";
   if (isset($_POST["type_question_name_".$ALX])) $typeQuestionName = trim(htmlspecialcharsEx($_POST["type_question_name_".$ALX]));?>
							<input type="hidden" id="type_question_name_<?=$ALX?>" name="type_question_name_<?=$ALX?>" value="<?=(strlen($typeQuestionName)>0) ? $typeQuestionName : $arResult["TYPE_QUESTION"][0]["NAME"];?>">
...
<?		echo bitrix_sessid_post()?>
		<div class="afbf_submit_block">
			<input type="submit" class="fb_close button button-secondary button-nuka" id="fb_close_<?=$ALX?>" name="SEND_FORM" value="<?=GetMessage('ALX_TP_MESSAGE_SUBMIT')?>" />
		</div>
	</form>
